...
#    rettxt = output = StringIO()
    ...
    print("buffjkfgyjdgryfdujdf-------------------------+++++++++++++++++++++++++++++++++++********************",buf)
    print("line++++++++++++++++++++++++++++++++++++++++",result)
...
"""print("going to check is there any pdf file or not")
for file in glob.glob(inputPDFDir + '*.pdf'):
    m = re.search(r'(.*)\.pdf$', file)
    print("extract txt from " + file)
    outputText('./resume/Nicholas Farino.pdf', m.group(1) + '.txt')"""

def call():
    inputPDFFile = './Linkedin/Abhisek Kundu.pdf'
    outputTXTFile = './resume/t.txt'
    result = outputText(inputPDFFile, outputTXTFile)
    print("h------------list comes")
    print(result)
    return result
...
# inputPDFFile = './Linkedin/Abhisek Kundu.pdf'
# outputTXTFile = './resume/q.txt'
# outputText(inputPDFFile, outputTXTFile)
call()